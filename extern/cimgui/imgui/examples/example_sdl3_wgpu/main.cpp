...
main(int, char **)
{
    ...
    SDL_Window *window           = SDL_CreateWindow(
        "Dear ImGui SDL3+WebGPU example", wgpu_surface_width, wgpu_surface_height, window_flags);
    if (window == nullptr)
    {
...
#endif
    {
        ...
        // function] React to changes in screen size
        int width, height;
        ...
        // Check surface status for error. If texture is not optimal, try to reconfigure the
        // surface.
...
#if defined(IMGUI_IMPL_WEBGPU_BACKEND_DAWN)
static WGPUAdapter
RequestAdapter(wgpu::Instance &instance)
{
...
static WGPUDevice
RequestDevice(wgpu::Instance &instance, wgpu::Adapter &adapter)
{
    ...
    auto         onRequestDevice =
        [&](wgpu::RequestDeviceStatus status, wgpu::Device local_device, wgpu::StringView message) {
...
static void
handle_request_adapter(WGPURequestAdapterStatus status,
                       WGPUAdapter              adapter,
...
static void
handle_request_device(WGPURequestDeviceStatus status,
                      WGPUDevice              device,
                      WGPUStringView          message,
                      void                   *userdata1,
                      void                   *userdata2)
...
static WGPUAdapter
RequestAdapter(WGPUInstance &instance)
{
...
// SDL3 helper to create a WebGPU surface (exclusively!) for Native/Desktop applications: available
// only together with WebGPU/WGPU backend As of today (2025/10) there is no "official" support in
// SDL3 to create a surface for WebGPU backend This stub uses "low level" SDL3 calls to acquire
// information from a specific Window Manager. Currently supported platforms: Windows / Linux (X11
// and Wayland) / MacOS. Not necessary nor available with EMSCRIPTEN.
#ifndef __EMSCRIPTEN__
...
WGPUSurface
CreateWGPUSurface(const WGPUInstance &instance, SDL_Window *window)
{
    ...
    else if (!SDL_strcmp(SDL_GetCurrentVideoDriver(), "x11"))
    {
        ...
        create_info.RawWindow
            = (void *)SDL_GetNumberProperty(propertiesID, SDL_PROP_WINDOW_X11_WINDOW_NUMBER, 0);
        create_info.RawDisplay = (void *)SDL_GetPointerProperty(
