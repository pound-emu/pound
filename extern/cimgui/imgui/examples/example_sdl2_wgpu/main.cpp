...
main(int, char **)
{
    ...
    SDL_Window *window = SDL_CreateWindow("Dear ImGui SDL2+WebGPU example",
...
// SDL2 helper to create a WebGPU surface (exclusively!) for Native/Desktop applications: available
// only together with WebGPU/WGPU backend As of today (2025/10/31) there is no "official" support in
// SDL2 to create a surface for WebGPU backend. This stub uses "low level" SDL2 calls to acquire
...
#include <SDL_syswm.h>
#undef Status // X11 headers are leaking this and also 'Success', 'Always', 'None', all used in DAWN
              // api. Add #undef if necessary.
