...
**/
static int16_t
sNetToNative16(const unsigned char *value)
{
...
**/
static int32_t
sNetToNative32(const unsigned char *value)
{
#ifdef USYNERGY_LITTLE_ENDIAN
...
**/
static void
sTrace(uSynergyContext *context, const char *text)
{
...
**/
static void
sAddString(uSynergyContext *context, const char *string)
{
...
**/
static void
sAddUInt8(uSynergyContext *context, uint8_t value)
{
...
**/
static void
sAddUInt16(uSynergyContext *context, uint16_t value)
{
...
**/
static void
sAddUInt32(uSynergyContext *context, uint32_t value)
{
    uint8_t *reply      = context->m_replyCur;
...
**/
static uSynergyBool
sSendReply(uSynergyContext *context)
{
    // Set header size
...
**/
static void
sSendMouseCallback(uSynergyContext *context)
{
...
**/
static void
sSendKeyboardCallback(uSynergyContext *context,
                      uint16_t         key,
                      uint16_t         modifiers,
                      uSynergyBool     down,
                      uSynergyBool     repeat)
...
**/
static void
sSendJoystickCallback(uSynergyContext *context, uint8_t joyNum)
{
    int8_t *sticks;
...
static void
sProcessMessage(uSynergyContext *context, const uint8_t *message)
{
    // We have a packet!
    ...
    else if (USYNERGY_IS_PACKET("DCLP"))
    {
        ...
        // remaining 'string' (as per the Synergy %s string format (which is 1 uint32 for size
        // followed by a char buffer (not necessarily null terminated)). 		1 uint32:
        //The number of formats present in the message
    ...
    else
    {
        ...
        char buffer[64];
        sprintf(
            buffer, "Unknown packet '%c%c%c%c'", message[4], message[5], message[6], message[7]);
        sTrace(context, buffer);
        return;
    }
...
**/
static void
sSetDisconnected(uSynergyContext *context)
{
    context->m_connected        = USYNERGY_FALSE;
...
**/
static void
sUpdateContext(uSynergyContext *context)
{
    ...
    if (packlen > USYNERGY_RECEIVE_BUFFER_SIZE)
    {
        ...
        while (num_received != packlen)
        {
            ...
            if (context->m_receiveFunc(
                    context->m_cookie, context->m_receiveBuffer, to_receive, &ditch_received)
                == USYNERGY_FALSE)
            {
                /* Receive failed, let's try to reconnect */
...
**/
void
uSynergyInit(uSynergyContext *context)
{
    /* Zero memory */
...
**/
void
uSynergyUpdate(uSynergyContext *context)
{
...
**/
void
uSynergySendClipboard(uSynergyContext *context, const char *text)
{
    ...
    if (text_length > max_length)
    {
        char buffer[128];
        sprintf(
            buffer, "Clipboard buffer too small, clipboard truncated at %d characters", max_length);
        sTrace(context, buffer);
    ...
    sAddUInt32(context,
               4 + 4 + 4
                   + text_length); /* Rest of message size: numFormats, format, length, data */
