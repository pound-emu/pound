...
template<typename T>
T
ImLerp(double a, double b, float t)
{
...
enum ImGuiItemStatusFlags_
{
    ...
    = 1 << 11, // Similar to ImGuiItemStatusFlags_Edited but bypassing ImGuiItemFlags_NoMarkEdited.
...
enum ImGuiNavMoveFlags_
{
    ...
    = 1 << 6, // Force scrolling to min/max (used by Home/End) // FIXME-NAV: Aim to remove or reword
              // as ImGuiScrollFlags
...
enum ImGuiDockNodeFlagsPrivate_
{
    ...
    = 1 << 11, // Saved // The central node has 2 main properties: stay visible when empty, only use
               // "remaining" spaces from its neighbor.
    ...
    = 1 << 18, //       // Any docked window will be automatically be focus-route chained
               //       (window->ParentWindowForFocusRoute set to this) so Shortcut() in this window
               //       can run when any docked window is focused.
    ...
    = 1
      << 21, //       // Disable this node from being docked over another window or non-empty node.
...
struct IMGUI_API ImGuiDockNode
{
    ...
    bool               IsVisible
        : 1; // Set to false when the node is hidden (usually disabled as it has no active window)
    bool IsFocused : 1;
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImGuiWindowBgClickFlags BgClickFlags
        : 8; // Configure behavior of click+dragging on window bg/void or over items. Default sets
             // by io.ConfigWindowsMoveFromTitleBarOnly. If you use this please report in #3379.
    ...
    // Docking
    bool DockIsActive
        : 1; // When docking artifacts are actually visible. When this is set, DockNode is
             // guaranteed to be != NULL. ~~ (DockNode != NULL) && (DockNode->Windows.Size > 1).
    bool DockNodeIsVisible : 1;
    bool DockTabIsVisible
        : 1; // Is our window visible this frame? ~~ is the corresponding tab selected?
...
IMGUI_API ImGuiID
DockBuilderSplitNode(ImGuiID  node_id,
                     ImGuiDir split_dir,
...
inline void
SetNextItemColorMarker(ImU32 col)
{
    ImGuiContext &g = *GImGui;
...
inline bool
operator==(const ImTextureRef &lhs, const ImTextureRef &rhs)
{
...
inline int
ImFontAtlasRectId_GetIndex(ImFontAtlasRectId id)
{
    return (id & ImFontAtlasRectId_IndexMask_);
}
inline unsigned int
ImFontAtlasRectId_GetGeneration(ImFontAtlasRectId id)
{
    return (unsigned int)(id & ImFontAtlasRectId_GenerationMask_)
           >> ImFontAtlasRectId_GenerationShift_;
}
inline ImFontAtlasRectId
ImFontAtlasRectId_Make(int index_idx, int gen_idx)
{
