...
void
ImGui::TextAligned(float align_x, float size_x, const char *fmt, ...)
{
    va_list args;
    va_start(args, fmt);
...
// (#7024)
void
ImGui::TextAlignedV(float align_x, float size_x, const char *fmt, va_list args)
{
    ImGuiWindow *window = GetCurrentWindow();
    if (window->SkipItems)
    {
...
bool
ImGui::CollapseButton(ImGuiID id, const ImVec2 &pos, ImGuiDockNode *dock_node)
{
    ImGuiContext &g      = *GImGui;
    ImGuiWindow  *window = g.CurrentWindow;
...
void
ImGui::ExtendHitBoxWhenNearViewportEdge(ImGuiWindow *window,
                                        ImRect      *bb,
                                        float        threshold,
                                        ImGuiAxis    axis)
{
...
void
ImGui::ImageWithBg(ImTextureRef  tex_ref,
...
void
ImGui::Image(ImTextureRef tex_ref, const ImVec2 &image_size, const ImVec2 &uv0, const ImVec2 &uv1)
{
...
void
ImGui::Image(ImTextureRef  tex_ref,
             const ImVec2 &image_size,
             const ImVec2 &uv0,
             const ImVec2 &uv1,
             const ImVec4 &tint_col,
...
void
ImGui::ShrinkWidths(ImGuiShrinkWidthItem *items, int count, float width_excess, float width_min)
{
    if (count == 1)
    {
...
ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect &bb, ImGuiComboFlags flags)
{
    ...
    if (!ret)
    {
        ...
        if (!g.IO.ConfigDebugBeginReturnValueOnce
            && !g.IO.ConfigDebugBeginReturnValueLoop) // Begin may only return false with those
...
static bool
TempInputIsClampEnabled(ImGuiSliderFlags flags,
                        ImGuiDataType    data_type,
                        const void      *p_min,
                        const void      *p_max)
{
...
// word-wrapping.
static int
STB_TEXTEDIT_MOVELINESTART_IMPL(ImGuiInputTextState      *obj,
        ...
        while (p < text_end)
        {
            ...
            if (p_eol == p_cursor
                && obj->LastMoveDirectionLR
                       != ImGuiDir_Left) // If we are already on a visible end-of-line, switch to
                                         // regular handle
            {
                break;
            }
...
void
ImGuiInputTextState::SetSelection(int start, int end)
{
    Stb->select_start = start;
    Stb->cursor = Stb->select_end = end;
}
float
ImGuiInputTextState::GetPreferredOffsetX() const
{
...
static int *
ImLowerBound(int *in_begin, int *in_end, int v)
{
    int *in_p = in_begin;
...
static int
InputTextLineIndexBuild(ImGuiInputTextFlags flags,
...
static ImVec2
InputTextLineIndexGetPosOffset(ImGuiContext        &g,
                               ImGuiInputTextState *state,
                               ImGuiTextIndex      *line_index,
                               const char          *buf,
                               const char          *buf_end,
            ...
            if (event_flag)
            {
                ...
                if (callback_data.BufDirty)
                {
                    ...
                    IM_ASSERT(
                        ...
                        == (int)ImStrlen(callback_data.Buf)); // You need to maintain BufTextLen if
                                                              // you change the text!
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            const float bg_offy_up
                = is_multiline ? 0.0f
                               : -1.0f; // FIXME-DPI: those offsets should be part of the style?
...
void
ImGui::TreeNodeSetOpen(ImGuiID storage_id, bool is_open)
{
    ImGuiContext &g       = *GImGui;
    ImGuiStorage *storage = g.CurrentWindow->DC.StateStorage;
...
// easy to increase.
static void
TreeNodeStoreStackData(ImGuiTreeNodeFlags flags, float x1)
{
    ImGuiContext &g      = *GImGui;
    ImGuiWindow  *window = g.CurrentWindow;
...
void
ImGui::TreeNodeDrawLineToChildNode(const ImVec2 &target_pos)
{
    ImGuiContext &g      = *GImGui;
    ImGuiWindow  *window = g.CurrentWindow;
...
void
ImGui::TreeNodeDrawLineToTreePop(const ImGuiTreeNodeStackData *data)
{
    ImGuiContext &g      = *GImGui;
    ImGuiWindow  *window = g.CurrentWindow;
...
ImGui::BeginMultiSelect(ImGuiMultiSelectFlags flags, int selection_size, int items_count)
{
    ...
    if (flags & ImGuiMultiSelectFlags_ScopeRect)
    {
        window->DC.CursorMaxPos
            = ms->ScopeRectMin; // CalcScopeRect() for ImGuiMultiSelectFlags_ScopeRect will measure
                                // in EndMultiSelect().
    }
...
ImGui::MultiSelectItemFooter(ImGuiID id, bool *p_selected, bool *p_pressed)
{
    ...
    if (hovered && IsMouseClicked(1)
        && (flags
            & (ImGuiMultiSelectFlags_NoAutoSelect | ImGuiMultiSelectFlags_NoSelectOnRightClick))
               == 0)
    {
...
ImGuiTabBar *
ImGui::TabBarFindByID(ImGuiID id)
{
    ImGuiContext &g = *GImGui;
...
void
ImGui::TabBarRemove(ImGuiTabBar *tab_bar)
{
    ImGuiContext &g = *GImGui;
...
void
ImGui::TabBarAddTab(ImGuiTabBar *tab_bar, ImGuiTabItemFlags tab_flags, ImGuiWindow *window)
{
    ImGuiContext &g = *GImGui;
...
ImVec2
ImGui::TabItemCalcSize(ImGuiWindow *window)
{
