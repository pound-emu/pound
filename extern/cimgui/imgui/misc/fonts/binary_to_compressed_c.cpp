...
main(int argc, char **argv)
{
    if (argc < 3)
    {
        ...
        printf(
            " -u32    = ~11 bytes of source per 4 bytes of data. 4 bytes in binary. Need "
            "endianness swapping on big-endian.\n");
        printf(
            " -base85 =  ~5 bytes of source per 4 bytes of data. 5 bytes in binary. Need "
            "decoder.\n");
        return 0;
    }
    ...
    bool ret = binary_to_compressed_c(
        argv[argn], argv[argn + 1], source_encoding, use_compression, use_static);
    if (!ret)
    {
                       ...
                       bool           use_static)
{
    ...
    else if (source_encoding == SourceEncoding_U8)
    {
        ...
        fprintf(
            out, "// Exported using binary_to_compressed_c.exe -u8 \"%s\" %s\n", filename, symbol);
        fprintf(out,
                ...
                (int)compressed_sz);
        fprintf(out,
                "%sconst unsigned char %s_%sdata[%d] =\n{",
                static_str,
                symbol,
                compressed_str,
                (int)compressed_sz);
    ...
    else if (source_encoding == SourceEncoding_U32)
    {
        // As integers
        fprintf(
            out, "// Exported using binary_to_compressed_c.exe -u32 \"%s\" %s\n", filename, symbol);
        fprintf(out,
                "%sconst unsigned int %s_%ssize = %d;\n",
                static_str,
                symbol,
