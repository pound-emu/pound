...
//  2025-06-12: ImGui_ImplOSX_HandleEvent() only process event for window containing our view.
//  (#8644) 2025-05-15: [Docking] Add Platform_GetWindowFramebufferScale() handler, to allow varying
//  Retina display density on multiple monitors. 2025-03-21: Fill gamepad inputs and set
...
- (void)updateImePosWithView:(NSView *)view
{
    ...
    if (io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable)
    {
        ...
        if (window.styleMask
            & NSWindowStyleMaskFullSizeContentView) // No title bar windows should be considered.
        {
            contentRect = frame;
        }
        ...
        _imeRect.origin.y       = firstScreenFrame.size.height - _imeRect.size.height
                                  - _imeRect.origin.y; // Opposite of ConvertNSRect()
    }
    else
    {
...
ImGui_ImplOSX_Init(NSView *view)
{
    ...
    // io.AddMouseViewportEvent() with correct data (optional)
    ...
    bd->MouseCursors[ImGuiMouseCursor_Wait] = bd->MouseCursors[ImGuiMouseCursor_Progress] =
        [NSCursor respondsToSelector:@selector(busyButClickableCursor)]
    ...
    platform_io.Platform_SetImeDataFn
        = [](ImGuiContext *, ImGuiViewport *, ImGuiPlatformImeData *data) -> void {
...
ImGui_ImplOSX_HandleEvent(NSEvent *event, NSView *view)
{
        ...
        || event.type == NSEventTypeRightMouseDragged || event.type == NSEventTypeOtherMouseDragged)
    {
        ...
        if (io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable)
        {
            ...
            mousePoint.y = CGDisplayPixelsHigh(kCGDirectMainDisplay)
        ...
        else
        {
            ...
            mousePoint = [view convertPoint:mousePoint
...
static void
ImGui_ImplOSX_CreateWindow(ImGuiViewport *viewport)
{
    ImGui_ImplOSX_Data         *bd = ImGui_ImplOSX_GetBackendData();
    ...
    if (floor(NSAppKitVersionNumber) > NSAppKitVersionNumber10_6
        && ceil(NSAppKitVersionNumber) < NSAppKitVersionNumber10_15)
    {
...
static void
ImGui_ImplOSX_DestroyWindow(ImGuiViewport *viewport)
{
...
static void
ImGui_ImplOSX_ShowWindow(ImGuiViewport *viewport)
{
...
static ImVec2
ImGui_ImplOSX_GetWindowPos(ImGuiViewport *viewport)
{
    ImGui_ImplOSX_ViewportData *vd = (ImGui_ImplOSX_ViewportData *)viewport->PlatformUserData;
    IM_ASSERT(vd->Window != 0);
    ...
    NSRect firstScreenFrame = NSScreen.screens[0].frame;
    return ImVec2(frame.origin.x,
                  firstScreenFrame.size.height - frame.origin.y - contentRect.size.height);
}

static void
ImGui_ImplOSX_SetWindowPos(ImGuiViewport *viewport, ImVec2 pos)
{
    ImGui_ImplOSX_ViewportData *vd = (ImGui_ImplOSX_ViewportData *)viewport->PlatformUserData;
    IM_ASSERT(vd->Window != 0);
...
static ImVec2
ImGui_ImplOSX_GetWindowSize(ImGuiViewport *viewport)
{
    ImGui_ImplOSX_ViewportData *vd = (ImGui_ImplOSX_ViewportData *)viewport->PlatformUserData;
    IM_ASSERT(vd->Window != 0);
...
static void
ImGui_ImplOSX_SetWindowSize(ImGuiViewport *viewport, ImVec2 size)
{
    ImGui_ImplOSX_ViewportData *vd = (ImGui_ImplOSX_ViewportData *)viewport->PlatformUserData;
    IM_ASSERT(vd->Window != 0);
...
static ImVec2
ImGui_ImplOSX_GetWindowFramebufferScale(ImGuiViewport *viewport)
{
    ImGui_ImplOSX_ViewportData *vd       = (ImGui_ImplOSX_ViewportData *)viewport->PlatformUserData;
    NSWindow                   *window   = vd->Window;
...
static void
ImGui_ImplOSX_SetWindowFocus(ImGuiViewport *viewport)
{
    ImGui_ImplOSX_Data         *bd = ImGui_ImplOSX_GetBackendData();
    ImGui_ImplOSX_ViewportData *vd = (ImGui_ImplOSX_ViewportData *)viewport->PlatformUserData;
    IM_ASSERT(vd->Window != 0);
...
static bool
ImGui_ImplOSX_GetWindowFocus(ImGuiViewport *viewport)
{
    ImGui_ImplOSX_ViewportData *vd = (ImGui_ImplOSX_ViewportData *)viewport->PlatformUserData;
    IM_ASSERT(vd->Window != 0);
...
static bool
ImGui_ImplOSX_GetWindowMinimized(ImGuiViewport *viewport)
{
    ImGui_ImplOSX_ViewportData *vd = (ImGui_ImplOSX_ViewportData *)viewport->PlatformUserData;
    IM_ASSERT(vd->Window != 0);
...
static void
ImGui_ImplOSX_SetWindowTitle(ImGuiViewport *viewport, const char *title)
{
    ImGui_ImplOSX_ViewportData *vd = (ImGui_ImplOSX_ViewportData *)viewport->PlatformUserData;
    IM_ASSERT(vd->Window != 0);
...
static void
ImGui_ImplOSX_SetWindowAlpha(ImGuiViewport *viewport, float alpha)
{
    ImGui_ImplOSX_ViewportData *vd = (ImGui_ImplOSX_ViewportData *)viewport->PlatformUserData;
    IM_ASSERT(vd->Window != 0);
...
static float
ImGui_ImplOSX_GetWindowDpiScale(ImGuiViewport *viewport)
{
    ImGui_ImplOSX_ViewportData *vd = (ImGui_ImplOSX_ViewportData *)viewport->PlatformUserData;
    IM_ASSERT(vd->Window != 0);
