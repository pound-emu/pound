...
//  2026-04-23: DirectX11: Added support for standard draw callbacks (in platform_io):
//  DrawCallback_ResetRenderState, DrawCallback_SetSamplerLinear, DrawCallback_SetSamplerNearest.
//  Obsoleting samplers from ImGui_ImplDX11_RenderState. (#9378) 2026-01-19: DirectX11: Added
//  'SamplerNearest' in ImGui_ImplDX11_RenderState. Renamed 'SamplerDefault' to 'SamplerLinear'.
...
//  it is not set under Windows). 2025-02-24: [Docking] Added undocumented
//  ImGui_ImplDX11_SetSwapChainDescs() to configure swap chain creation for secondary viewports.
...
struct ImGui_ImplDX11_Data
{
    ...
    ImGui_ImplDX11_RenderState
        *RenderState; // == (ImGui_ImplDX11_RenderState*)ImGui::GetPlatformIO().Renderer_RenderState
                      // during rendering.
...
ImGui_ImplDX11_DrawCallback_ResetRenderState(const ImDrawList *, const ImDrawCmd *)
{
  ...
  // implement this way.
static void
ImGui_ImplDX11_DrawCallback_SetSamplerLinear(const ImDrawList *, const ImDrawCmd *)
{
    ImGui_ImplDX11_Data *bd = ImGui_ImplDX11_GetBackendData();
    bd->RenderState->DeviceContext->PSSetSamplers(0, 1, &bd->pTexSamplerLinear);
}
static void
ImGui_ImplDX11_DrawCallback_SetSamplerNearest(const ImDrawList *, const ImDrawCmd *)
{
    ImGui_ImplDX11_Data *bd = ImGui_ImplDX11_GetBackendData();
...
ImGui_ImplDX11_UpdateTexture(ImTextureData *tex)
{
    ...
    else if (tex->Status == ImTextureStatus_WantUpdates)
    {
        ...
        {
            D3D11_BOX box
                = { (UINT)r.x, (UINT)r.y, (UINT)0, (UINT)(r.x + r.w), (UINT)(r.y + r.h), (UINT)1 };
...
// Multi-Viewports: configure templates used when creating swapchains for secondary viewports. Will
// try them in order. This is intentionally not declared in the .h file yet, so you will need to
// copy this declaration:
void ImGui_ImplDX11_SetSwapChainDescs(const DXGI_SWAP_CHAIN_DESC *desc_templates,
                                      int                         desc_templates_count);
void
ImGui_ImplDX11_SetSwapChainDescs(const DXGI_SWAP_CHAIN_DESC *desc_templates,
                                 int                         desc_templates_count)
{
    ImGui_ImplDX11_Data *bd = ImGui_ImplDX11_GetBackendData();
...
{
    // The main viewport (owned by the application) will always have RendererUserData == nullptr
...
static void
ImGui_ImplDX11_SetWindowSize(ImGuiViewport *viewport, ImVec2 size)
{
    ImGui_ImplDX11_Data         *bd = ImGui_ImplDX11_GetBackendData();
...
static void
ImGui_ImplDX11_RenderWindow(ImGuiViewport *viewport, void *)
{
    ImGui_ImplDX11_Data         *bd = ImGui_ImplDX11_GetBackendData();
    ImGui_ImplDX11_ViewportData *vd = (ImGui_ImplDX11_ViewportData *)viewport->RendererUserData;
...
static void
ImGui_ImplDX11_SwapBuffers(ImGuiViewport *viewport, void *)
{
    ImGui_ImplDX11_ViewportData *vd = (ImGui_ImplDX11_ViewportData *)viewport->RendererUserData;
    if (vd->SwapChain)
    {
