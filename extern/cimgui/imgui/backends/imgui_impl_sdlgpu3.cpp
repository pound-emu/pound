...
//  support for dynamic font atlas (ImGuiBackendFlags_RendererHasTextures). [X] Renderer:
...
//  Obsoleting samplers from ImGui_ImplSDLGPU3_RenderState. (#9378) 2026-03-19: Fixed issue in
//  ImGui_ImplSDLGPU3_DestroyTexture() if ImTextureID_Invalid is defined to be != 0, which became
//  the default since 2026-03-12. (#9295, #9310) 2026-02-25: Removed unnecessary call to
//  SDL_WaitForGPUIdle when releasing vertex/index buffers. (#9262) 2025-11-26: macOS version can
//  use MSL shaders in order to support macOS 10.14+ (vs Metallib shaders requiring macOS 14+).
//  Requires calling SDL_CreateGPUDevice() with SDL_GPU_SHADERFORMAT_MSL. 2025-09-18: Call
//  platform_io.ClearRendererHandlers() on shutdown. 2025-08-20: Added
//  ImGui_ImplSDLGPU3_InitInfo::SwapchainComposition and ImGui_ImplSDLGPU3_InitInfo::PresentMode to
//  configure how secondary viewports are created. 2025-08-08: *BREAKING* Changed ImTextureID type
//  from SDL_GPUTextureSamplerBinding* to SDL_GPUTexture*, which is more natural and easier for user
//  to manage. If you need to change the current sampler, you can access the
//  ImGui_ImplSDLGPU3_RenderState struct. (#8866, #8163, #7998, #7988) 2025-08-08: Expose
//  SamplerDefault and SamplerCurrent in ImGui_ImplSDLGPU3_RenderState. Allow callback to change
//  sampler. 2025-06-25: Mapping transfer buffer for texture update use cycle=true. Fixes artifacts
//  e.g. on Metal backend. 2025-06-11: Added support for ImGuiBackendFlags_RendererHasTextures, for
//  dynamic font atlas. Removed ImGui_ImplSDLGPU3_CreateFontsTexture() and
//  ImGui_ImplSDLGPU3_DestroyFontsTexture(). 2025-04-28: Added support for special
//  ImDrawCallback_ResetRenderState callback to reset render state. 2025-03-30: Made
//  ImGui_ImplSDLGPU3_PrepareDrawData() reuse GPU Transfer Buffers which were unusually slow to
//  recreate every frame. Much faster now. 2025-03-21: Fixed typo in function name
                      ...
                      SDL_GPUBufferUsageFlags usage)
{
    ...
    // There is no need for calling SDL_WaitForGPUIdle here, as SDL3 will handle deferred buffer
...
// Draw callbacks
static void
ImGui_ImplSDLGPU3_DrawCallback_ResetRenderState(const ImDrawList *, const ImDrawCmd *)
{
} // Intentionally empty. Used as an identifier for rendering loop to call its code. Simpler to
  // implement this way.
static void
ImGui_ImplSDLGPU3_DrawCallback_SetSamplerLinear(const ImDrawList *, const ImDrawCmd *)
{
    ImGui_ImplSDLGPU3_Data *bd = ImGui_ImplSDLGPU3_GetBackendData();
    bd->CurrentSampler         = bd->TexSamplerLinear;
}
static void
ImGui_ImplSDLGPU3_DrawCallback_SetSamplerNearest(const ImDrawList *, const ImDrawCmd *)
{
    ImGui_ImplSDLGPU3_Data *bd = ImGui_ImplSDLGPU3_GetBackendData();
    bd->CurrentSampler         = bd->TexSamplerNearest;
}
    ...
    for (const ImDrawList *draw_list : draw_data->CmdLists)
    {
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                // Draw
                // **IF YOU GET A CRASH HERE** In 1.92.2 on 2025/08/08 we have changed ImTextureID
                // to store 'SDL_GPUTexture*' instead of storing 'SDL_GPUTextureSamplerBinding'. Any
                // code loading custom texture using this backend needs to be updated.
...
static void
ImGui_ImplSDLGPU3_DestroyTexture(ImTextureData *tex)
{
    ImGui_ImplSDLGPU3_Data *bd = ImGui_ImplSDLGPU3_GetBackendData();
...
void
ImGui_ImplSDLGPU3_UpdateTexture(ImTextureData *tex)
{
    ImGui_ImplSDLGPU3_Data     *bd = ImGui_ImplSDLGPU3_GetBackendData();
    ImGui_ImplSDLGPU3_InitInfo *v  = &bd->InitInfo;
...
ImGui_ImplSDLGPU3_DestroyDeviceObjects()
{
    ...
    {
        SDL_ReleaseGPUSampler(v->Device, bd->TexSamplerNearest);
...
static void
ImGui_ImplSDLGPU3_CreateWindow(ImGuiViewport *viewport)
{
    ...
    SDL_SetGPUSwapchainParameters(data->InitInfo.Device,
                                  window,
...
static void
ImGui_ImplSDLGPU3_RenderWindow(ImGuiViewport *viewport, void *)
{
    ImGui_ImplSDLGPU3_Data *data = ImGui_ImplSDLGPU3_GetBackendData();
    SDL_Window *window = SDL_GetWindowFromID((SDL_WindowID)(intptr_t)viewport->PlatformHandle);
    ...
    if (swapchain_texture != nullptr)
    {
        ImGui_ImplSDLGPU3_PrepareDrawData(
            draw_data, command_buffer); // FIXME-OPT: Not optimal, may this be done earlier?
...
static void
ImGui_ImplSDLGPU3_DestroyWindow(ImGuiViewport *viewport)
{
    ImGui_ImplSDLGPU3_Data *data = ImGui_ImplSDLGPU3_GetBackendData();
