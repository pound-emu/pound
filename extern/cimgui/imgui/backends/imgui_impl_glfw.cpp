...
//  headers are not accessible. (#9225) 2026-01-25: [Docking] Improve workarounds for cases where
//  GLFW is unable to provide any reliable monitor info. Preserve existing monitor list when none of
//  the new one is valid. (#9195, #7902, #5683) 2026-01-18: [Docking] Dynamically load X11 functions
//  to avoid -lx11 linking requirement introduced on 2025-09-10. 2025-12-12: Added
//  IMGUI_IMPL_GLFW_DISABLE_X11 / IMGUI_IMPL_GLFW_DISABLE_WAYLAND to forcefully disable either.
//  2025-12-10: Avoid repeated glfwSetCursor()/glfwSetInputMode() calls when unnecessary. Lowers
//  overhead for very high framerates (e.g. 10k+ FPS). 2025-11-06: Lower minimum requirement to
//  GLFW 3.0. Though a recent version e.g GLFW 3.4 is highly recommended. 2025-09-18: Call
//  platform_io.ClearPlatformHandlers() on shutdown. 2025-09-15: Content Scales are always reported
//  as 1.0 on Wayland. FramebufferScale are always reported as 1.0 on X11. (#8920, #8921)
//  2025-09-10: [Docking] Improve multi-viewport behavior in tiling WMs on X11 via the
//  ImGui_ImplGlfw_SetWindowFloating() function. Note: using GLFW backend on Linux/BSD etc. requires
//  linking with -lX11. (#8884, #8474, #8289) 2025-07-08: Made
...
static void
ImGui_ImplGlfw_ContextMap_Add(GLFWwindow *window, ImGuiContext *ctx)
{
    g_ContextMap.push_back(ImGui_ImplGlfw_WindowToContext { window, ctx });
}
static void
ImGui_ImplGlfw_ContextMap_Remove(GLFWwindow *window)
{
...
static ImGuiContext *
ImGui_ImplGlfw_ContextMap_Get(GLFWwindow *window)
{
    for (ImGui_ImplGlfw_WindowToContext &entry : g_ContextMap)
    {
        if (entry.Window == window)
...
typedef int (*PFN_XChangeWindowAttributes)(Display *,
                                           Window,
                                           unsigned long,
...
static ImGui_ImplGlfw_Data *
ImGui_ImplGlfw_GetBackendData(GLFWwindow *window)
{
...
static void
ImGui_ImplGlfw_UpdateKeyModifiers(ImGuiIO &io, GLFWwindow *window)
{
...
static bool
ImGui_ImplGlfw_ShouldChainCallback(ImGui_ImplGlfw_Data *bd, GLFWwindow *window)
{
...
static float
ImGui_ImplWin32_GetLegacyDpiScale()
{
...
static void
glfwGetWindowContentScale(GLFWwindow *, float *x_scale, float *y_scale)
{
    *x_scale = *y_scale = ImGui_ImplWin32_GetLegacyDpiScale();
}
static void
glfwGetMonitorContentScale(GLFWmonitor *, float *x_scale, float *y_scale)
{
    *x_scale = *y_scale = ImGui_ImplWin32_GetLegacyDpiScale();
}
...
static void
ImGui_ImplGlfw_GetWindowSizeAndFramebufferScale(GLFWwindow *window,
                                                ImVec2     *out_size,
...
static void
ImGui_ImplGlfw_SetWindowFloating(ImGui_ImplGlfw_Data *bd, GLFWwindow *window)
{
#ifdef GLFW_EXPOSE_NATIVE_X11
...
static ImVec2
ImGui_ImplGlfw_GetWindowFramebufferScale(ImGuiViewport *viewport)
{
    ImGui_ImplGlfw_ViewportData *vd = (ImGui_ImplGlfw_ViewportData *)viewport->PlatformUserData;
...
static int
ImGui_ImplGlfw_CreateVkSurface(ImGuiViewport *viewport,
                               ImU64          vk_instance,
...
ImGui_ImplGlfw_WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (msg)
    {
        ...
        case WM_NCHITTEST: {
            ...
            // ImGuiViewportFlags_NoInputs flag is set while dragging a viewport, as want to detect
            // the window behind the one we are dragging. If you cannot easily access those viewport
            // flags from your windowing/event code: you may manually synchronize its state e.g. in
            // your main loop after calling UpdatePlatformWindows(). Iterate all viewports/platform
