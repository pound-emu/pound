...
//  [!] Renderer: User texture binding. Use a VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE 'VkDescriptorSet' as
//  texture identifier. Call ImGui_ImplVulkan_AddTexture() to register one. Read the FAQ about
//  ImTextureID/ImTextureRef + https://github.com/ocornut/imgui/pull/914 for discussions. [X]
//  Renderer: Large meshes support (64k+ vertices) even with 16-bit indices
//  (ImGuiBackendFlags_RendererHasVtxOffset). [X] Renderer: Texture updates support for dynamic font
//  atlas (ImGuiBackendFlags_RendererHasTextures). [X] Renderer: Expose selected render state for
...
//  (#9378) 2026-04-22: *BREAKING CHANGE* redesigned to use separate ImageView + Sampler instead of
//  Combined Image Sampler. This change allows us to facilitate changing samplers, in line with
//  other backends.
//              - When registering custom textures: changed ImGui_ImplVulkan_AddTexture() signature
//              to remove Sampler.
...
//              - When creating your own descriptor pool (instead of letting backend creates its
//              own):
//                - Before: need at least IMGUI_IMPL_VULKAN_MINIMUM_IMAGE_SAMPLER_POOL_SIZE
//                descriptors of type VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER.
//                - After:  need at least IMGUI_IMPL_VULKAN_MINIMUM_SAMPLED_IMAGE_POOL_SIZE
//                descriptors of type VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE.
//                                      + IMGUI_IMPL_VULKAN_MINIMUM_SAMPLER_POOL_SIZE descriptors of
//                                      type VK_DESCRIPTOR_TYPE_SAMPLER.
//  2026-03-11: Vulkan: Added ImGui_ImplVulkan_PipelineInfo::ExtraDynamicStates[] to allow
//  specifying extra dynamic states to add when creating the VkPipeline. (#9211) 2025-09-26:
//  [Helpers] *BREAKING CHANGE*: Vulkan: Helper ImGui_ImplVulkanH_DestroyWindow() does not call
//  vkDestroySurfaceKHR(): as surface is created by caller of
//  ImGui_ImplVulkanH_CreateOrResizeWindow(), it is more consistent that we don't destroy it.
//  (#9163) 2026-01-05: [Helpers] *BREAKING CHANGE*: Vulkan: Helper for creating render pass uses
//  ImGui_ImplVulkanH_Window::AttachmentDesc to create render pass. Removed ClearEnabled. (#9152)
//  2025-11-24: [Helpers] Vulkan: Helper for creating a swap-chain (used by examples and
//  multi-viewports) selects VkSwapchainCreateInfoKHR's compositeAlpha based on
//  cap.supportedCompositeAlpha. (#8784) 2025-11-13: [Docking] Handle viewport surface creation
//  failure without crashing. (#9068) 2025-10-15: Vulkan: Added IMGUI_IMPL_VULKAN_VOLK_FILENAME to
//  configure path to volk.h header. (#9008) 2025-09-26: *BREAKING CHANGE*: moved some fields in
//  ImGui_ImplVulkan_InitInfo: init_info.RenderPass --> init_info.PipelineInfoMain.RenderPass,
...
//  recently so shouldn't affect many users. 2025-09-26: [Helpers] *BREAKING CHANGE*: Helper
...
//  2025-09-22: [Docking] Added ImGui_ImplVulkanH_GetWindowDataFromViewport() accessor/helper.
//  (#8946, #8940) 2025-09-18: Call platform_io.ClearRendererHandlers() on shutdown. 2025-09-04:
//  Vulkan: Added ImGui_ImplVulkan_CreateMainPipeline(). (#8110, #8111) 2025-07-27: Vulkan: Fixed
//  texture update corruption introduced on 2025-06-11. (#8801, #8755, #8840) 2025-07-07: Vulkan:
//  Fixed texture synchronization issue introduced on 2025-06-11. (#8772) 2025-06-27: Vulkan: Fixed
//  validation errors during texture upload/update by aligning upload size to 'nonCoherentAtomSize'.
//  (#8743, #8744) 2025-06-11: Vulkan: Added support for ImGuiBackendFlags_RendererHasTextures, for
//  dynamic font atlas. Removed ImGui_ImplVulkan_CreateFontsTexture() and
//  ImGui_ImplVulkan_DestroyFontsTexture(). 2025-05-07: Vulkan: Fixed validation errors during
//  window detach in multi-viewport mode. (#8600, #8176) 2025-05-07: Vulkan: Load dynamic rendering
//  functions using vkGetDeviceProcAddr() + try both non-KHR and KHR versions. (#8600, #8326, #8365)
//  2025-04-07: Vulkan: Deep-copy ImGui_ImplVulkan_InitInfo::PipelineRenderingCreateInfo's
//  pColorAttachmentFormats buffer when set, in order to reduce common user-error of specifying a
//  pointer to data that gets out of scope. (#8282) 2025-02-14: *BREAKING CHANGE*: Added uint32_t
//  api_version to ImGui_ImplVulkan_LoadFunctions(). 2025-02-13: Vulkan: Added ApiVersion field in
...
//  [Helpers] Vulkan: Added more ImGui_ImplVulkanH_XXXX helper functions to simplify our examples.
//  2024-12-11: [Helpers] Vulkan: Fixed setting VkSwapchainCreateInfoKHR::preTransform for platforms
//  not supporting VK_SURFACE_TRANSFORM_IDENTITY_BIT_KHR. (#8222) 2024-11-27: Vulkan: Make
//  user-provided descriptor pool optional. As a convenience, when setting
//  init_info->DescriptorPoolSize the backend will create one itself. (#8172, #4867) 2024-10-07:
//  Vulkan: Changed default texture sampler to Clamp instead of Repeat/Wrap. 2024-10-07: Vulkan:
//  Expose selected render state in ImGui_ImplVulkan_RenderState, which you can access in 'void*
//  platform_io.Renderer_RenderState' during draw callbacks. 2024-10-07: Vulkan: Compiling with
//  '#define ImTextureID=ImU64' is unnecessary now that dear imgui defaults ImTextureID to u64
//  instead of void*. 2024-04-19: Vulkan: Added convenience support for Volk via
//  IMGUI_IMPL_VULKAN_USE_VOLK define (you can also use IMGUI_IMPL_VULKAN_NO_PROTOTYPES + wrap Volk
//  via ImGui_ImplVulkan_LoadFunctions().) 2024-02-14: *BREAKING CHANGE*: Moved RenderPass parameter
//  from ImGui_ImplVulkan_Init() function to ImGui_ImplVulkan_InitInfo structure. Not required when
//  using dynamic rendering. 2024-02-12: *BREAKING CHANGE*: Dynamic rendering now require filling
//  PipelineRenderingCreateInfo structure. 2024-01-19: [Helpers] Vulkan: Fixed
//  vkAcquireNextImageKHR() validation errors in VulkanSDK 1.3.275 by allocating one extra semaphore
//  than in-flight frames. (#7236) 2024-01-11: Vulkan: Fixed vkMapMemory() calls unnecessarily using
//  full buffer size (#3957). Fixed MinAllocationSize handing (#7189). 2024-01-03: Vulkan: Added
//  MinAllocationSize field in ImGui_ImplVulkan_InitInfo to workaround zealous "best practice"
//  validation layer. (#7189, #4238) 2024-01-03: Vulkan: Stopped creating command pools with
//  VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT as we don't reset them. 2023-11-29: Vulkan:
//  Fixed mismatching allocator passed to vkCreateCommandPool() vs vkDestroyCommandPool(). (#7075)
//  2023-11-10: *BREAKING CHANGE*: Removed parameter from ImGui_ImplVulkan_CreateFontsTexture():
//  backend now creates its own command-buffer to upload fonts.
...
//  IMGUI_VK_QUEUED_FRAMES 2). Added ImGui_ImplVulkan_SetMinImageCount(). 2019-04-04: [Helpers]
//  Vulkan: Added VkInstance argument to ImGui_ImplVulkanH_CreateWindow() optional helper.
//  2019-04-04: Vulkan: Avoid passing negative coordinates to vkCmdSetScissor, which debug
//  validation layers do not like. 2019-04-01: Vulkan: Support for 32-bit index buffer (#define
//  ImDrawIdx unsigned int). 2019-02-16: Vulkan: Viewport and clipping rectangles correctly using
//  draw_data->FramebufferScale to allow retina display. 2018-11-30: Misc: Setting up
//  io.BackendRendererName so it can be displayed in the About Window. 2018-08-25: [Helpers] Vulkan:
//  Fixed mishandled VkSurfaceCapabilitiesKHR::maxImageCount=0 case. 2018-06-22: Inverted the
//  parameters to ImGui_ImplVulkan_RenderDrawData() to be consistent with other backends.
//  2018-06-08: Misc: Extracted imgui_impl_vulkan.cpp/.h away from the old combined GLFW+Vulkan
//  example. 2018-06-08: Vulkan: Use draw_data->DisplayPos and draw_data->DisplaySize to setup
//  projection matrix and clipping rectangle. 2018-03-03: [Helpers] Vulkan: Various refactor,
//  created a couple of ImGui_ImplVulkanH_XXX helper that the example can use and that viewport
//  support will use. 2018-03-01: Vulkan: Renamed ImGui_ImplVulkan_Init_Info to
//  ImGui_ImplVulkan_InitInfo and fields to match more closely Vulkan terminology. 2018-02-16: Misc:
//  Obsoleted the io.RenderDrawListsFn callback, ImGui_ImplVulkan_Render() calls
//  ImGui_ImplVulkan_RenderDrawData() itself. 2018-02-06: Misc: Removed call to ImGui::Shutdown()
                                  ...
                                  int                                  fb_height)
{
    ...
    // Setup sampler
    vkCmdBindDescriptorSets(bd->RenderState->CommandBuffer,
...
// Draw callbacks
static void
ImGui_ImplVulkan_DrawCallback_ResetRenderState(const ImDrawList *, const ImDrawCmd *)
{
} // Intentionally empty. Used as an identifier for rendering loop to call its code. Simpler to
  // implement this way.
static void
ImGui_ImplVulkan_DrawCallback_SetSamplerLinear(const ImDrawList *, const ImDrawCmd *)
{
    ImGui_ImplVulkan_Data *bd = ImGui_ImplVulkan_GetBackendData();
    vkCmdBindDescriptorSets(bd->RenderState->CommandBuffer,
                            VK_PIPELINE_BIND_POINT_GRAPHICS,
...
static void
ImGui_ImplVulkan_DrawCallback_SetSamplerNearest(const ImDrawList *, const ImDrawCmd *)
{
    ImGui_ImplVulkan_Data *bd = ImGui_ImplVulkan_GetBackendData();
    vkCmdBindDescriptorSets(bd->RenderState->CommandBuffer,
                            VK_PIPELINE_BIND_POINT_GRAPHICS,
...
static void
ImGui_ImplVulkan_CreateDescriptorSetLayout(VkDescriptorSetLayout *p_layout,
...
ImGui_ImplVulkan_CreateSamplerDS(VkSampler sampler)
{
    ImGui_ImplVulkan_Data     *bd = ImGui_ImplVulkan_GetBackendData();
    ImGui_ImplVulkan_InitInfo *v  = &bd->VulkanInitInfo;
...
ImGui_ImplVulkan_CreateDeviceObjects()
{
    ...
    if (!bd->PipelineLayout)
    {
        ...
        VkDescriptorSetLayout set_layout[2]
            = { bd->DescriptorSetLayoutTexture, bd->DescriptorSetLayoutSampler };
...
VkDescriptorSet
ImGui_ImplVulkan_AddTexture(VkImageView image_view, VkImageLayout image_layout)
{
    ImGui_ImplVulkan_Data     *bd   = ImGui_ImplVulkan_GetBackendData();
    ImGui_ImplVulkan_InitInfo *v    = &bd->VulkanInitInfo;
                                       ...
                                       VkImageUsageFlags            image_usage)
{
    ...
    ImGui_ImplVulkanH_CreateWindowSwapChain(
        physical_device, device, wd, allocator, width, height, min_image_count, image_usage);
                                ...
                                const VkAllocationCallbacks *allocator)
{
    ...
    // vkDestroySurfaceKHR(instance, wd->Surface, allocator); // v1.92.6 (~2026-01-16): because
    // wd->Surface is user provided we don't attempt to destroy it ourself.
}
...
void
ImGui_ImplVulkanH_DestroyAllViewportsRenderBuffers(VkDevice                     device,
                                                   const VkAllocationCallbacks *allocator)
{
    ImGuiPlatformIO &platform_io = ImGui::GetPlatformIO();
...
ImGui_ImplVulkanH_GetWindowDataFromViewport(ImGuiViewport *viewport)
{
...
static void
ImGui_ImplVulkan_CreateWindow(ImGuiViewport *viewport)
{
    ImGui_ImplVulkan_Data         *bd = ImGui_ImplVulkan_GetBackendData();
    ...
    wd->SurfaceFormat = ImGui_ImplVulkanH_SelectSurfaceFormat(v->PhysicalDevice,
                                                              ...
                                                              requestSurfaceImageFormats.Size,
    ...
    wd->PresentMode = ImGui_ImplVulkanH_SelectPresentMode(
        v->PhysicalDevice, wd->Surface, &present_modes[0], IM_COUNTOF(present_modes));
    // printf("[vulkan] Secondary window selected PresentMode = %d\n", wd->PresentMode);

    // Create SwapChain, RenderPass, Framebuffer, etc.
    wd->UseDynamicRendering   = v->UseDynamicRendering;
    wd->AttachmentDesc.loadOp = (viewport->Flags & ImGuiViewportFlags_NoRendererClear)
...
static void
ImGui_ImplVulkan_DestroyWindow(ImGuiViewport *viewport)
{
...
static void
ImGui_ImplVulkan_SetWindowSize(ImGuiViewport *viewport, ImVec2 size)
{
    ImGui_ImplVulkan_Data         *bd = ImGui_ImplVulkan_GetBackendData();
    ImGui_ImplVulkan_ViewportData *vd = (ImGui_ImplVulkan_ViewportData *)viewport->RendererUserData;
...
ImGui_ImplVulkan_RenderWindow(ImGuiViewport *viewport, void *)
{
    ...
    {
        {
            ...
            if (err == VK_ERROR_OUT_OF_DATE_KHR)
            {
                vd->SwapChainNeedRebuild
                    = true; // Since we are not going to swap this frame anyway, it's ok that
    ...
    ImGui_ImplVulkan_RenderDrawData(
        viewport->DrawData, fd->CommandBuffer, bd->PipelineForViewports);
...
static void
ImGui_ImplVulkan_SwapBuffers(ImGuiViewport *viewport, void *)
{
    ImGui_ImplVulkan_Data         *bd = ImGui_ImplVulkan_GetBackendData();
    ImGui_ImplVulkan_ViewportData *vd = (ImGui_ImplVulkan_ViewportData *)viewport->RendererUserData;
    if (vd == nullptr)
