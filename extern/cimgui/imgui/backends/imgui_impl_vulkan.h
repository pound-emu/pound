...
// ImGui_ImplVulkan_LoadFunctions().) (When using Volk from directory outside your include
// directories list you can specify full path to the volk.h header,
//  for example when using Volk from VulkanSDK and using
//  include_directories(${Vulkan_INCLUDE_DIRS})' from 'find_package(Vulkan REQUIRED)')
...
struct ImGui_ImplVulkan_PipelineInfo
{
    ...
    ImVector<VkDynamicState>
        ExtraDynamicStates; // Optional, allows to insert more dynamic states into our VkPipeline
    ...
    VkImageUsageFlags
        SwapChainImageUsage; // Extra flags for vkCreateSwapchainKHR() calls for secondary
                             // viewports. We automatically add VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT.
                             // You can add e.g. VK_IMAGE_USAGE_TRANSFER_SRC_BIT if you need to
                             // capture from viewports.
};
...
struct ImGui_ImplVulkan_InitInfo
{
    ...
    uint32_t DescriptorPoolSize;   // Optional: set to create internal ImageView descriptor pool
    ...
    ImGui_ImplVulkan_PipelineInfo
        PipelineInfoForViewports; // Infos for Secondary Viewports (created by backend)
    ...
    // - if .sType == VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO we use specified structs,
...
IMGUI_IMPL_API VkDescriptorSet ImGui_ImplVulkan_AddTexture(VkImageView   image_view,
                                                           VkImageLayout image_layout);
IMGUI_IMPL_API void            ImGui_ImplVulkan_RemoveTexture(VkDescriptorSet descriptor_set);

#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
IMGUI_IMPL_API VkDescriptorSet ImGui_ImplVulkan_AddTexture(
    VkSampler sampler, VkImageView image_view, VkImageLayout image_layout); // Ignore VkSampler
#endif
...
IMGUI_IMPL_API ImGui_ImplVulkanH_Window *ImGui_ImplVulkanH_GetWindowDataFromViewport(
    ImGuiViewport *viewport); // Access to Vulkan objects associated with a viewport (e.g to export
                              // a screenshot)
...
struct ImGui_ImplVulkanH_Window
{
    ...
    VkClearValue
        ClearValue; // RenderPass creation: clear value when using VK_ATTACHMENT_LOAD_OP_CLEAR.
    ...
    ImGui_ImplVulkanH_Window()
    {
        ...
        PresentMode
            = VK_PRESENT_MODE_MAX_ENUM_KHR; // Ensure we get an error if user doesn't set this.
