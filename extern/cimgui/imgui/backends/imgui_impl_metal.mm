...
//  2026-04-28: Added support for standard draw callbacks (in platform_io):
//  DrawCallback_SetSamplerLinear and DrawCallback_SetSamplerNearest. (#9378, #9381) 2026-04-23:
//  Added support for standard draw callbacks (in platform_io): DrawCallback_ResetRenderState
//  (others are not yet supported). (#9378) 2026-04-14: Metal: use a dedicated bufferCacheLock to
//  avoid crashing when bufferCache is replaced by a new object while being used for @synchronize().
//  (#9367) 2026-04-03: Metal: avoid redundant vertex buffer bind in SetupRenderState. (#9343)
//  2026-03-19: Fixed issue in ImGui_ImplMetal_RenderDrawData() if ImTextureID_Invalid is defined to
//  be != 0, which became the default since 2026-03-12. (#9295, #9310) 2025-09-18: Call
//  platform_io.ClearRendererHandlers() on shutdown. 2025-06-11: Added support for
//  ImGuiBackendFlags_RendererHasTextures, for dynamic font atlas. Removed
//  ImGui_ImplMetal_CreateFontsTexture() and ImGui_ImplMetal_DestroyFontsTexture(). 2025-02-03:
//  Metal: Crash fix. (#8367) 2025-01-08: Metal: Fixed memory leaks when using metal-cpp (#8276,
//  #8166) or when using multiple contexts (#7419). 2022-08-23: Metal: Update deprecated property
...
// Draw callbacks
static void
ImGui_ImplMetal_DrawCallback_ResetRenderState(const ImDrawList *, const ImDrawCmd *)
{
} // Intentionally empty. Used as an identifier for rendering loop to call its code. Simpler to
  // implement this way.
static void
ImGui_ImplMetal_DrawCallback_SetSamplerLinear(const ImDrawList *, const ImDrawCmd *)
{
    ImGui_ImplMetal_Data *bd = ImGui_ImplMetal_GetBackendData();
    [bd->RenderCommandEncoder setFragmentSamplerState:bd->SharedMetalContext.samplerStateLinear
                                              atIndex:0];
}
static void
ImGui_ImplMetal_DrawCallback_SetSamplerNearest(const ImDrawList *, const ImDrawCmd *)
{
    ImGui_ImplMetal_Data *bd = ImGui_ImplMetal_GetBackendData();
    [bd->RenderCommandEncoder setFragmentSamplerState:bd->SharedMetalContext.samplerStateNearest
                                              atIndex:0];
}
                               ...
                               id<MTLRenderCommandEncoder> commandEncoder)
{
    ...
    ImGui_ImplMetal_SetupRenderState(
        draw_data, commandBuffer, commandEncoder, renderPipelineState, vertexBuffer, 0);
    ...
    for (const ImDrawList *draw_list : draw_data->CmdLists)
    {
        ...
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            if (pcmd->UserCallback)
            {
                ...
                {
                    ImGui_ImplMetal_SetupRenderState(draw_data,
                                                     commandBuffer,
                                                     commandEncoder,
                                                     renderPipelineState,
                                                     vertexBuffer,
...
ImGui_ImplMetal_UpdateTexture(ImTextureData *tex)
{
    ...
    if (tex->Status == ImTextureStatus_WantCreate)
    {
        ...
        id<MTLTexture> texture =
            [bd->SharedMetalContext.device newTextureWithDescriptor:textureDescriptor];
        [texture
            replaceRegion:MTLRegionMake2D(0, 0, (NSUInteger)tex->Width, (NSUInteger)tex->Height)
              mipmapLevel:0
    ...
    else if (tex->Status == ImTextureStatus_WantUpdates)
    {
        ...
        {
            [backend_tex.metalTexture replaceRegion:MTLRegionMake2D((NSUInteger)r.x,
                                                                    (NSUInteger)r.y,
                                                                    (NSUInteger)r.w,
                                                                    (NSUInteger)r.h)
                                        mipmapLevel:0
...
ImGui_ImplMetal_CreateWindow(ImGuiViewport *viewport)
{
    ...
    // PlatformHandleRaw should always be a NSWindow*, whereas PlatformHandle might be a
    // higher-level handle (e.g. GLFWWindow*, SDL_Window*). Some back-ends will leave
    // PlatformHandleRaw == 0, in which case we assume PlatformHandle will contain the NSWindow*.
    void *handle
...
ImGui_ImplMetal_RenderWindow(ImGuiViewport *viewport, void *)
{
    ...
    {
        // Do not render windows which are completely occluded. Calling -[CAMetalLayer nextDrawable]
        // will hang for approximately 1 second if the Metal layer is completely occluded.
        return;
    }
