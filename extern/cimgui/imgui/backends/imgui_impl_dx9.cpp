...
//  IMGUI_USE_BGRA_PACKED_COLOR support, as this is the optimal color encoding for DirectX9. [X]
...
//  (#9378) 2026-03-19: Fixed issue in ImGui_ImplDX9_UpdateTexture() if ImTextureID_Invalid is
//  defined to be != 0, which became the default since 2026-03-12. (#9295, #9310) 2025-09-18: Call
//  platform_io.ClearRendererHandlers() on shutdown. 2025-06-11: DirectX9: Added support for
//  ImGuiBackendFlags_RendererHasTextures, for dynamic font atlas. 2024-10-07: DirectX9: Changed
//  default texture sampler to Clamp instead of Repeat/Wrap. 2024-02-12: DirectX9: Using RGBA format
//  when supported by the driver to avoid CPU side conversion. (#6575) 2022-10-11: Using 'nullptr'
...
//  bd->XXXX). 2021-06-25: DirectX9: Explicitly disable texture state stages after >= 1. 2021-05-19:
//  DirectX9: Replaced direct access to ImDrawCmd::TextureId with a call to ImDrawCmd::GetTexID().
//  (will become a requirement) 2021-04-23: DirectX9: Explicitly setting up more graphics states to
//  increase compatibility with unusual non-default states. 2021-03-18: DirectX9: Calling
//  IDirect3DStateBlock9::Capture() after CreateStateBlock() as a workaround for state restoring
//  issues (see #3857). 2021-03-03: DirectX9: Added support for IMGUI_USE_BGRA_PACKED_COLOR in
//  user's imconfig file. 2021-02-18: DirectX9: Change blending equation to preserve alpha in output
//  buffer. 2019-05-29: DirectX9: Added support for large mesh (64K+ vertices), enable
...
//  erroneous assert in ImGui_ImplDX9_InvalidateDeviceObjects(). 2019-01-16: Misc: Disabled fog
...
//  exposed ImGui_ImplDX9_RenderDrawData() in the .h file so you can call it yourself. 2018-02-06:
...
// Draw callbacks
static void
ImGui_ImplDX9_DrawCallback_ResetRenderState(const ImDrawList *, const ImDrawCmd *)
{
} // Intentionally empty. Used as an identifier for rendering loop to call its code. Simpler to
  // implement this way.
static void
ImGui_ImplDX9_DrawCallback_SetSamplerLinear(const ImDrawList *, const ImDrawCmd *)
{
    ImGui_ImplDX9_Data *bd = ImGui_ImplDX9_GetBackendData();
...
static void
ImGui_ImplDX9_DrawCallback_SetSamplerNearest(const ImDrawList *, const ImDrawCmd *)
{
    ImGui_ImplDX9_Data *bd = ImGui_ImplDX9_GetBackendData();
...
ImGui_ImplDX9_RenderDrawData(ImDrawData *draw_data)
{
    ...
    if (!bd->pVB || bd->VertexBufferSize < draw_data->TotalVtxCount)
    {
        ...
        if (device->CreateVertexBuffer((UINT)bd->VertexBufferSize * sizeof(CUSTOMVERTEX),
    ...
    if (!bd->pIB || bd->IndexBufferSize < draw_data->TotalIdxCount)
    {
        ...
        if (device->CreateIndexBuffer((UINT)bd->IndexBufferSize * sizeof(ImDrawIdx),
                                      D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY,
    ...
    // When using multi-viewports, it appears that there's an odd logic in DirectX9 which prevent
    // subsequent windows from rendering until the first window submits at least one draw call, even
    // once. That's our workaround. (see #2560)
...
ImGui_ImplDX9_CopyTextureRegion(
    bool tex_use_colors, const ImU32 *src, int src_pitch, ImU32 *dst, int dst_pitch, int w, int h)
{
...
void
ImGui_ImplDX9_UpdateTexture(ImTextureData *tex)
{
    ImGui_ImplDX9_Data *bd = ImGui_ImplDX9_GetBackendData();
    ...
    else if (tex->Status == ImTextureStatus_WantUpdates)
    {
        ...
        RECT               update_rect = { (LONG)tex->UpdateRect.x,
...
static void
ImGui_ImplDX9_CreateWindow(ImGuiViewport *viewport)
{
    ImGui_ImplDX9_Data         *bd = ImGui_ImplDX9_GetBackendData();
...
static void
ImGui_ImplDX9_DestroyWindow(ImGuiViewport *viewport)
{
...
static void
ImGui_ImplDX9_SetWindowSize(ImGuiViewport *viewport, ImVec2 size)
{
    ImGui_ImplDX9_Data         *bd = ImGui_ImplDX9_GetBackendData();
...
static void
ImGui_ImplDX9_RenderWindow(ImGuiViewport *viewport, void *)
{
    ImGui_ImplDX9_Data         *bd = ImGui_ImplDX9_GetBackendData();
    ImGui_ImplDX9_ViewportData *vd = (ImGui_ImplDX9_ViewportData *)viewport->RendererUserData;
...
static void
ImGui_ImplDX9_SwapBuffers(ImGuiViewport *viewport, void *)
{
    ImGui_ImplDX9_ViewportData *vd = (ImGui_ImplDX9_ViewportData *)viewport->RendererUserData;
