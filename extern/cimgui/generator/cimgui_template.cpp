...
CIMGUI_API void
ImGuiTextBuffer_appendf(ImGuiTextBuffer *self, const char *fmt, ...)
{
    va_list args;
    va_start(args, fmt);
...
CIMGUI_API void
ImVector_ImWchar_destroy(ImVector_ImWchar *self)
{
    IM_DELETE(self);
}

CIMGUI_API void
ImVector_ImWchar_Init(ImVector_ImWchar *p)
{
    IM_PLACEMENT_NEW(p) ImVector<ImWchar>();
}
CIMGUI_API void
ImVector_ImWchar_UnInit(ImVector_ImWchar *p)
{
    p->~ImVector<ImWchar>();
}
...
// NOTE: Some function pointers in the ImGuiPlatformIO structure are not C-compatible because of
// their use of a complex return type. To work around this, we store a custom CimguiStorage object
// inside ImGuiIO::BackendLanguageUserData, which contains C-compatible function pointer variants
// for these functions. When a user function pointer is provided, we hook up the underlying
// ImGuiPlatformIO function pointer to a thunk which accesses the user function pointer through
// CimguiStorage.
...
// Gets a reference to the CimguiStorage object stored in the current ImGui context's
...
ImVec2
Platform_GetWindowPos_hook(ImGuiViewport *vp)
{
    ImVec2 pos;
...
CIMGUI_API void
ImGuiPlatformIO_Set_Platform_GetWindowPos(ImGuiPlatformIO *platform_io,
...
ImVec2
Platform_GetWindowSize_hook(ImGuiViewport *vp)
{
    ImVec2 size;
...
CIMGUI_API void
ImGuiPlatformIO_Set_Platform_GetWindowSize(ImGuiPlatformIO *platform_io,
                                           void (*user_callback)(ImGuiViewport *vp,
                                                                 ImVec2        *out_size))
{
    CimguiStorage &storage              = GetCimguiStorage();
