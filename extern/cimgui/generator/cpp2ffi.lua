...
local function parse_enum_value(value, allenums, dontpost)
    ...
    elseif type(value) == "string" then
        ...
        --already in allenums
        if allenums[clean(value)] then
            return allenums[clean(value)]
        end
        ...
        while (#seps > 0) do
            ...
            if sep == v then
                ...
                if allenums[val1] then
                    val1 = allenums[val1]
                end
                if allenums[val2] then
                    val2 = allenums[val2]
                end
                --clean 1u
                if type(val1) == "string" then
                    val1 = val1:gsub("(%d)(u)$", "%1")
                end
                if type(val2) == "string" then
                    val2 = val2:gsub("(%d)(u)$", "%1")
                end
                --for getting numbers from "1ULL"
                if type(val1) == "string" then
                    val1 = loadstring("return " .. val1)()
                end
                if type(val2) == "string" then
                    val2 = loadstring("return " .. val2)()
                end
                if v == "~" then
        ...
        if type(several[1]) == "cdata" then
            several[1] = tostring(several[1])
        end
        return several[1]
    end
end
...
local function name_overloadsAlgo(v)
    ...
    for i, t in ipairs(v) do
        ...
        aa[i] = {}
        local num = 1
        for typec in signature:gsub("(%(.-%))", function(x)
            return x:gsub(",", "\0")
        end)                  :gmatch(".-([^,]+),?") do
...
--]]
    ...
    for i, ar in ipairs(argsTa) do
        local ttype, template, te, code2 = check_template(ar) --ar:match("([^%s,%(%)]+)%s*<(.-)>")
        if template then
            if self.typenames[stname] ~= template then
                --rule out template typename
                self.templates[ttype] = self.templates[ttype] or {}
                self.templates[ttype][template] = te
            end
        end
    ...
    if not ret and stname then
        --must be constructors
        if not (stname == funcname or "~" .. stname == funcname) then
            --break end
    ...
    for numcdef, t in ipairs(FP.funcdefs) do
        ...
        for i, arg in ipairs(defT.argsT) do
            if arg.type == "ImTextureID" then
                print(t.cimguiname, t.signature);
                hasTextureID = true;
                break
            end
        end
...
local function gen_structs_c(FP)
    ...
    local npsorted = {}
    for k, n in pairs(nP_used_sorted) do
        insert(npsorted, k)
    end
    table.sort(npsorted, function(a, b)
        return (nP_used_sorted[a] < nP_used_sorted[b]) or ((nP_used_sorted[a] == nP_used_sorted[b]) and (a < b))
    end)
    --M.prtable(npsorted)
    --error"DEUG"
    ----------
...
local function get_nonPODused(FP)
    ...
    for k, defs in pairs(FP.defsT) do
        ...
        for i, w in ipairs(defs) do
            if not w.templated then
                --and not w.location:match"internal" then
...
local function ADDdestructors(FP)
    ...
    for numcdef, t in ipairs(FP.funcdefs) do
        if t.cimguiname then
            ...
            if not defT[1].ret and not defT[1].constructor then
                --if constructor not processed
...
function M.Parser()
    ...
    function par.getCname(stname, funcname, namespace)
        ...
        if pre:match ":" then
            print(stname, funcname, namespace);
            error "debug"
        end
        -- if stname== "" then
    ...
    local function get_cdefs(gccline, locat, cdef)
        ...
        for line in M.location(pipe, { locat }) do
            ...
            skip = false
            if line ~= "" then
                if line:match("^%s*static const") and not line:match("static const int") then
                    skip = true
                end
                if not skip then
                ...
                if key and value then
                    ...
                    if key then
                        ...
                        if key then
                            ...
                            linet = linet:gsub("%((" .. key .. ")%)", "()")
                            self.typedefs_dict[key] = linet
                        elseif not line:match "typedef%s*struct" then
                            --discard typedef struct
    ...
    local function derived_check(it)
        ...
        if inistruct:match ":" then
            ...
            if not stname then
                stname, derived = inistruct:match "class%s*([^%s:]+):(.+)"
            end
            if derived then
                derived = derived:match "(%S+)$"
            else
                assert(inistruct:match "private" or inistruct:match "protected", inistruct)
            end
        else
        ...
        for i, it in ipairs(itsarr) do
            ...
            if not isLeaf(it.re_name) then
                ...
                if it.re_name == "struct_re" then
                    ...
                    if templa1 or templa2 then
                        print("template found", stname, templa1, templa2, "typename", typename)
                    end

                    if typename or templa2 then
                        -- it is a struct template
    ...
    function par:clean_structR1(itst, doheader)
        ...
        if derived then
            print(stname, "derived from", derived)
        end
        ...
        for j, it in ipairs(itlist) do
            if (it.re_name == "vardef_re" or it.re_name == "functype_re") then
                -- or it.re_name == "union_re") then
                if not (it.re_name == "vardef_re" and it.item:match "static") then
                    ...
                    if template then
                        ...
                        if self.typenames[ttype] and self.typenames[ttype] ~= template and self.typenames[ttype] .. "*" ~= template then
                            --rule out T (template typename)
                            self.templates[ttype] = self.templates[ttype] or {}
                            self.templates[ttype][template] = te
                            it2 = code2
                        end
    ...
    function par:enum_for_header(it, outtab)
        ...
        if enumname then
            ...
            --unamed enum just repeat declaration
        ...
        local processer = function(it)
                ...
                if not it.parent or it.parent.re_name == "namespace_re" then
                    ...
                    elseif it.re_name == "functypedef_re" then
                        ...
                        if it2:match("using") then
                            ...
                            if assign and assign:match "%(%s*%*%s*%)" then
                                --function typedef
                    ...
                    if it.re_name == "vardef_re" and it.item:match "^%s*struct" then
                        ...
                        if it.parent then
                            --must be struct name; inside namespace
            ...
            elseif it.re_name == "struct_re" or it.re_name == "typedef_st_re" or it.re_name == "class_re" then
                if it.opaque_struct then
                    ...
                    if it.parent then
                        --and (it.parent.re_name == "struct_re" or it.parent.re_name == "typedef_st_re" then
                        ...
                        if embededst then
                            --discards false which can happen with untagged structs
    ...
    function par:parse_struct_line(line, outtab, comment)
        ...
        if line == "" then
            table.insert(outtab, { type = "nil", name = "nil" });
            return
        end
        line = clean_spaces(line)
    ...
    function par:gen_structs_and_enums_table()
        ...
        local processer = function(it)
            ...
            elseif it.re_name == "struct_re" or it.re_name == "typedef_st_re" or it.re_name == "class_re" then
                ...
                if it.name then
                    outtab.opaque_structs[it.name] = it.opaque_struct
                end
                --if not void stname or templated
                if not structname then
                    print("NO NAME", cleanst, it.item)
            ...
            for i, t in ipairs(struct) do
                ...
                if val then
                    ...
                    elseif allenums[val] then
                        ...
                        if not f then
                            print("error on loadstring", err, "with val:", val)
                        end
                        f()
                        t.size = estevalor
                    end
    ...
    function par:dump_alltypes()
        for k, v in pairs(self.alltypes) do
            print(k, typetoStr(k))
        end
    end
    ...
    function par:gentemplatetypedef(ttype, te, newte)
        ...
        if self.templates_done[ttype][te] then
            return ""
        end
...
-- end
M.serializeTableF = function(t)
    return tb2st_serialize(t, { pretty = true })
end --new serialization more compact
    ...
    local function location_it()
        repeat
            ...
            elseif in_location then
                ...
                if keepemptylines or line:match("%S") then
                    --nothing on emptyline
...
local function func_header_generate_structs(FP)
    ...
    table_do_sorted(FP.embeded_enums, function(k, v)
        table.insert(outtab, "typedef " .. v .. " " .. k .. ";\n")
    end)
    --table.insert(outtab, "\n//////////templates\n")
        ...
        if t.cimguiname then
            ...
            if not custom and not manual and not def.templated and not FP.get_skipped(def) and
                ...
                elseif def.destructor then
                    ...
                    local ret = def.ret
                    if def.stname == "" or def.is_static_function then
                        --ImGui namespace or top level
