...
bool     gui_plugin_load_module(gui_plugin_t *POUND_RESTRICT plugin,
                                const char *POUND_RESTRICT   source_path);
void     gui_plugin_destroy(gui_plugin_t *plugin);
bool     gui_exports_get(gui_exports_t *out);
uint64_t file_modified_time(const char *path);
bool     copy_file(const char *POUND_RESTRICT source, const char *POUND_RESTRICT destination);
